...
const useTwitter = (props) => {
  ...
  const TWModal = useCallback(
    (modalProps) => {
      const onWebViewStateChanged = (webViewNavigation) => {
        const {canGoBack, url} = webViewNavigation;
        consoleLog('eee', webViewNavigation);
        if (canGoBack && url.indexOf('twittersdk://?denied') > -1) {
          setVisible(false);
          if (props?.onCancel) {
            props.onCancel();
          }
        ...
        // setVisible(false);
        // } else {
        // console.log('onWebViewStateChanged.....');
        // consoleLog('onWebViewStateChanged.....');
  ...
  useEffect(() => {
    if (webViewState) {
      ...
      // consoleLog(webViewState, 'webviewState');
      ...
      client
        ...
        .then((response) => {
          props.onSuccess(response, {
            ...
            oauth_token_secret: client.TokenSecret,
          });
          setLoggedIn(false);
        })
        .catch((err) => {
          console.warn(
            `[get("account/verify_credentials.json") failed] ${err}`,
          );
          if (props?.onError) {
            props.onError(err);
          }
