...
export default class ListMyPostExpire extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  async getData() {
    try {
      let {page} = this.state;
      let dataPost = {page};
      dataPost.user_id = this.props.user_id;
      dataPost.type = this.props.type;
      let rs = await services.getExpirePosts(dataPost);
      this.setState({
        data: rs.data.data,
